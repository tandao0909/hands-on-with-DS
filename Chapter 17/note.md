...
During training, the generator and discriminator constantly compete with each other in a *zero-sum* game, which is a game where the only way of winning is make your opponent losing. As training advances, the game may end up in a state where game theorists called a *Nash equilibrium*, name after the mathematician John Nash: this is when no player can achieve better result by changing their own strategy, assuming other players do not change theirs. 

For example, a Nash equilibrium is reached when everyone drive on the left side of the road: not driver will be better off drive on the right side. Of course, there exists a second Nash equilibrium: when everyone drive on the right side of the road. Different initial states and dynamics may lead to one equilibrium or the other. In this example, there's a single optimal strategy once an equilibrium is reached (driving on the same side as everyone else), but a Nash equilibrium may involve multiple competing strategies (e.g., a predator tries to chase its prey, the prey tries to escape, and neither would be better off switching their strategy).

How does all of these has anything to do with GANs? Well, the author of GANs demonstrated that a GAN can only reached a single Nash equilibrium: that's when the generator produces perfectly realistic images, and the discriminator is force to guess (50% true, 50% false). This fact is very encouraging: it would seem that you juts need to train a GAN for long enough (not very pragmatic), and it will eventually reach this equilibrium, giving you a perfect generator. Unfortunately, it's not that simple: nothing guarantees that such an equilibrium will ever be reached.

The biggest difficulty is called *mode collapse*: this is when the generator's outputs gradually becomes less diverse. How can this happen? Suppose the generator gets a bit better at producing fake shoes images than any other classes. It will fool the discriminator a bit more with shoes, and the generator will start to think: "Hey, shoes is easy to trick, why don't I focus on it?", and it will produce even more images of shoes. Gradually, it forgets about how to produce any image of all other classes. Meanwhile, the only images the discriminator sees are shoes, so it also forgets how to distinguish fake images form all other classes. Eventually, when the discriminator manages to discriminate the fake shoes from the real ones, the generator will be force to move to another class. It may then be good at shirts, forgetting about the shoes, and the discriminator will follow. The GAN may gradually cycle across a few classes, never really becoming very good at any of them.

Moreover, because the generator and the discriminator are constantly pushing against each other, their parameters may end up oscillating and becoming unstable. Training may begin properly, and suddenly diverge for no apparent reason, due to these instabilities. And since many factors affect these complex dynamics, GANs are very sensitive to the hyperparameters: you may have to spend a lot of time and efforts to tweak the hyperparameters properly. In fact, the author fist used the Nadam optimizer, but he ran into a sever collapse, so he must switch to the RMSProp.

These problems have kept researchers very busy since 2014: many papers have been published on this topic, some proposing new cost functions (see this [GitHub repo](https://github.com/hwalsuklee/tensorflow-generative-model-collections) for a nice comparison), though a [2018 paper](https://arxiv.org/abs/1711.10337) by Google researchers judge their efficiency, or techniques to stabilize training or avoid the mode collapse issue. 

For example, a popular technique called *experience replay* works by storing the images produced by the generator at each iteration in a replay buffer (gradually dropping the older generated images) and training the discriminator using real images plus fake images sampled drawn from this buffer, instead of just the fake images generated by the current generator. This reduces the chance the discriminator will overfit the latest generator's outputs. Another common technique is called *mini-batch discrimination*: it measures how similar images are across the batch and provides this statistic to the discriminator, so it can easily reject a whole batch of fake images that lack diversity. This encourages the generator to produce a greater variety of images, reducing the chance of mode collapse. Other papers simply propose specific architectures that happens to work well.

In short, this is still a very active area of research, and the dynamics of GANs are still not perfectly understood. But the good news is that great progress has been made, and some of the results are truly astounding! We'll look at some of the most successful architectures, begin with deep convolutional GANs, which were the state  of the art just a few years ago.

## Deep Convolutional GANs

The author of the original GAN paper experimented with convolutional layers, but only tried to generate small images. Soon after, many researchers tried to build GANs based on deeper convolutional nets for larger images. This proved to be tricky, as training was very unstable, but [Alec Radfold el al.](https://arxiv.org/abs/1511.06434) finally succeed in late 2015, after experimenting with many different architectures and hyperparameters. They called their architecture *deep convolutional GANs* (DCGANs). Here are them main guidelines they proposed for building a stable convolutional GAN:
- Replace any pooling layers with stride convolutions (in the discriminator) and transpose convolutions (in the generator).
...
- Removing fully connected hidden layers for deeper architecture.
- Use ReLU activation in the generator for every layer except the output layers, which should use tanh.
- Use leaky ReLU activation in the discriminator for all layers.

These guidelines work fine for most use cases, but not always, so you may need to experiment with different hyperparameters. In fact, juts changing the random seed, and training the exact same model may sometimes work. Here is a small DCGAN that works reasonably well with Fashion MNIST: The generator takes codings of size 6,272 dimensions (7 * 7 * 128), and reshapes the result into a $7 \times 7 \times 128$ tensors. These tensors are then batch normalized, and fed to a transposed convolutional layer with a stride of 2, which upsamples these tensors from $7 \times 7$ to $14 \times 14$ and reduces its depth from 128 to 64. The results are fed to a transposed convolutional layer with a stride of 2 again, which upsamples these tensors from $14 \times 14$ to $28 \times 28$ and reduces its depth from 64 to 32. This layer uses the tanh activation function, so the outputs will range from -1 to 1. For this reason, before training the GAN, we need to rescale the training set to that same range. We also need to add an additional dimension for the channel.

The discriminator looks like a regular CNN for binary classification, except instead of using max pooling layers to downsample the image, we use strided convolutions (`strides=2`). Note that we use the leaky ReLU activation function. Overall, we respect the DCGAN guidelines, except we replaced the `BatchNormalization` layers in the discriminator with `Dropout` layers; otherwise, training was unstable in this case. Feel free to tweak this architecture: you will see how sensitive it is to the hyperparameters, especially the relative learning rates of the two networks.

Lastly, we build the dataset and then compile and train this model as earlier. After 50 epochs of training, the generator produces images that look fairly realistic. It's not perfect, but some of these images are pretty convincing.

If you scale up this architecture and train it on a large dataset of faces, you can get fairly realistic images. In fact, DCGANs can learn quite meaningful latent representations, as you can see below: many images were generated, and nine of them were picked manually (top left), including three representing men with glasses, three men without glasses, and three women without glasses. For each of these categories, the codings that were used to generate these images were averaged, and an image was generated based on the resulting mean codings (lower left). In short, each of the three lower images represents the mean of the three images above it. But this is not a simple mean computed at the pixel level (this would result in three overlapping faces), it is a mean computed in the latent space, so the images still look like normal faces. Amazingly, if you compute men with glasses, minus men without glasses, plus women without glasses - where term corresponds to one of the mean codings - and you generate an image that corresponds to this coding, you get an image at the center of the $3 \times 3$ grid of faces on the right: a woman with glasses! The eight other images around it were generate based on the same codings, plus a bit of noise, to illustrate the semantic interpolation capabilities of DCGANs. Being able to do arithmetics on faces feels like magic!

![Vector arithmetic for visual concepts (part of figure 7 from the DCGAN paper)](image-6.png)

DCGANs aren't perfect, though. For example, when you try to generate very large images using DCGANs, you often end up with locally convincing features but overall inconsistencies, such as shits with one sleeves much longer than the other, different earrings, or eyes looking opposite directions.

If you add each image's class as an extra input to both the generator and the discriminator, they will both learn what each class looks like , and thus you will be able to control the class of each image produced by the generator. This is called a [*conditional GAN*](https://arxiv.org/abs/1411.1784)(CGAN).


## Progressive Growing of GANs

In a [2018 paper](), Nvidia researchers Tero Kerras et al. proposed an important technique: they suggested generating small images at the beginning of training, then gradually adding convolutional layers to both the generator and the discriminator to produce larger and larger images ($4 \times 4$, $8 \times 8$, $16 \times 16$, ...,$512 \times 512$, $1,024 \times 1,024$). This approach resembles the greedy layer-wise approach of stacked autoencoder. The extra layers get added at the end of the generator and the start of the discriminator, and previously trained layers remain trainable.

For example, when growing the generator's outputs from $4 \times 4 $ to $8 \times 8$ (see the figure below), an upsampling layer (using nearest neighbor sampling) is added to the existing convolutional layer ("Conv 1") to produce $8 \times 8$ feature maps. These are fed to the new convolutional layer ("Conv 2"), which in turn feeds into a new output convolutional layer. To avoid breaking weights of the trained Conv 1, we gradually fade in two new convolutional layers (represented with dashed lines) and fade out the original output layer. The final outputs are a weighted sum of the new outputs (with weight $\alpha$) and the original outputs (with weight $1 - \alpha$), slowly increasing $\alpha$ from 0 to 1. A similar fade-in/fade-out technique is used when a new convolutional layer is added to the discriminator (followed by an average pooling layer for downsampling). Note that all convolutional layers use the `"same"` padding and strides of 1, so they preserve the height and width of their inputs. Only the downsampling and upsampling layers affect the spatial size. This includes the original convolutional layer, so it now produces $8 \times 8$ outputs (since its outputs are now $8 \times 8$). Lastly, the output layer uses kernel size of 1. They just project their inputs down to the desired number of color channels (typically 3).

![A progressively growing GAN: a GAN generator outputs 4 × 4 color images (left); we extend it to output 8 × 8 images (right)](image-7.png)

The paper also introduced some other techniques aimed at increasing the diversity of the outputs (to avoid mode collapse) and making training more stable:
- *Mini-batch standard deviation layer*: Added near the end of the discriminator. For each position in the inputs, it computes the standard deviation across all channels and all instances in the batch (`S = tf.math.reduce_std(inputs, axis=[0, -1])`), which it is calculated independently for each pixel in the image. These standard deviations are then averaged across all points to get a single value (`v = tf.math.reduce_mean(S)`). Finally, an extra feature map is added to each instance in the batch and filled with the computed value (`tf.concat([inputs, tf.fill([batch_size, height, width, 1], v)], axis=-1)`). If the generator produces images with little variety, then there will be a small standard deviation across feature maps in the discriminator. Thanks to this layer, the discriminator will have easy access to this statistic, making it less likely to be fooled by a generator that produces too little diversity. This will encourage the generator to produce more diverse outputs, reducing the risk of mode collapse.
- *Equalized learning rate*: Initialized all weights using a Gaussian distribution with mean 0 and standard deviation 1 rather than using He initialization. However, the weights are scaled down at runtime (i.e., every time the layer is executed) by the same factor as in He initialization: they are divided by $\sqrt{\dfrac{2}{n_\text{inputs}}}$, where $n_\text{inputs}$ is the number of inputs to the layer. The paper demonstrated that this technique significantly improved the GAN's performance when using adaptive gradient optimizers, such RMSProp and Adam. Indeed, these optimizers normalize the gradient updates by their estimated standard deviation (discussed in chapter 11), so parameters with a larger dynamic range (i.e., the ratio between the highest value and the lowest value) may take longer to train, while parameters with a lower dynamic range may be updated too quickly, leading to instabilities. By rescaling the weights as part of the model itself rather than just rescaling them upon initialization, this approach ensures that the dynamic range is the same for all parameters throughout training, so they all learn at the same speed. This both speeds up and stabilizes training.
- *Pixel-wise normalization layer*: Add after each convolutional layer in the generator. It normalizes each activation based on all the activations in the same image and the same location, but across all channel, by dividing by the square root of the mean squared activation. In TensorFlow code, this is `inputs / tf.sqrt(tf.reduce_mean(tf.square(X), axis=-1, keepdims=True) + 1e-8)` (the smoothing term `1e-8` is needed to avoid division by zero). This technique avoids explosions in the activations due to excessive competition between the generator and the discriminator.

The combination of all these techniques allowed the authors to generate [extremely convincing fake high-definition images of faces](https://www.youtube.com/watch?v=G06dEcZ-QTg). But how do we measure "convincing"? Evaluation is one of the big challenges when working with GANs: although it is possible to automatically evaluate the diversity of the generated images, judging their quality is a much trickier and subjective task. One technique is to use human rates, but this is time-consuming and costly. So, the authors proposed to measure the similarity between the local image structure of the generated images and the training images, at every scale. This ides led them to another groundbreaking innovation: StyleGANs.

## StyleGANs

The state of the art in high-resolution image generation was advanced once again by the same Nvidia team in a [2018 paper](https://arxiv.org/abs/1812.04948) that introduced the popular StyleGAN architecture. The authors used *style transfer* technique (discussed in the last exercise in chapter 14) in the generator to ensure the generated images have the same local structure as the training images, at every scale, greatly improving the quality of the generated images. The discriminator and the loss function are unchanged, only the generator. A StyleGAN generator is composed of two networks:
- *Mapping network*: An eight-layer MLP that maps the latent representations $\textbf{z}$ (i.e., the codings) to a vector $\textbf{w}$. This vector is then sent through multiple *affine transformations* (i.e., `Dense` layers with no activation functions, represented by the "A" boxes in the figure below), which produces multiple vectors. These vectors control the style of the generated images at different levels, from fine-grained texture (e.g., hair color) to high-level features (e.g., adult or child). In short, the mapping network maps the codings to multiple style vectors.
- *Synthesis network*: responsible for generating new images. It has a constant learned input (to be precise, this input will be constant after training, but during training, it will be tweaked by backpropagation). It processes this inputs through multiple convolutional and upsampling layers, as earlier, but there are two twists. First, some noise is added to the input and to all the outputs of the convolutional layers (before the activation function). Second, each noise layer is followed by an *adaptive instance normalization* (AdaIN) layer: it standardizes each feature map independently, by subtracting its mean and dividing by its standard deviation, then it uses the style vector to determine the scale and offset of each feature map (the style vector consists one scale and one bias term for each feature map).

![StyleGAN’s generator architecture (part of Figure 1 from the StyleGAN paper)](image-8.png)

The idea of adding some noise independently from the codings is very important. Some parts of an image are quite  random, such as the exact same position of each freckle or hair. In earlier GANs, this randomness ahd to either come form the codings or be some pseudorandom noise produced by the generator itself. If it come from the codings, it means that the generator had to dedicate a significant portion of the codings representational power to storing noise, which is not a good use of resource. Moreover, the noise had to be able to flow through the network and reach the final layers of the generator: this seem like an unnecessary constraint that will slow down training. And finally, some visual artifacts may appear because the same random noise was used at different levels. If instead the generator tried to generate the noise itself (which is pseudorandom), this noise might not look very convincing, leading to more visual artifacts. Plus, part of the generator's weights would be dedicated to generating pseudorandom noises, which is again wasteful. By adding extra noise input, all of these problems are avoided; the GAN is able to use the provided noise to add the right amount of stochastic to each part of the image.

The added noise is different at each level. Each noise input consists of a single feature map full of Gaussian noise, which is broadcast to all feature maps (of the given level), and scaled using learned per-feature scaling factors (this is represented as "B" boxes) before it is added.

Finally, StyleGANuses a technique called *mixing regularization* (or *style mixing*), where a percentage of the generated images are produced using two different codings. Specifically, the codings $\textbf{c}_1$ and $\textbf{c}_2$ are sent through the mapping network, giving two vectors $\textbf{w}_1$ and $\textbf{w}_2$. Then the synthesis network generates an image based on the styles $\textbf{w}_1$ for the first levels and the styles $\textbf{w}_2$ for the remaining levels. The cutoff level is picked randomly. This prevents the network from assuming that styles at adjacent levels are correlated, which in turn encourages locality in the GAN, meaning that each style vector only affects a limited number of traits in the generated image.

There's such a wide variety of GANs out there that it would require a whole book to cover them all. You cna go ahead and implement your own GAN, and it's normal to having trouble learning at first: it just requires a bit patience to get it working, but the result is worth it. If you're struggling with the implementation detail, there are plenty of Keras or TensorFlow implementations that you can look at. In fact, if you all want is to get some amazing results quickly, then you can just use a pretrained model (e.g., there are pretrained StyleGAN models available for Keras).

# Diffusion Models

The ideas behind diffusion models have been around for may years, but they were fist formalized in their modern form in a [2015 paper](https://arxiv.org/abs/1503.03585) by Jascha Sohl-Dickstein et al. from Stanford University and UC Berkeley. The authors applied tools from thermodynamics to model a diffusion process, similar to a drop of milk diffusing in a cup of tea. The core idea is to train a model to learn the reserve process: start from the the completely mixed state, and gradually "unmix" the milk from the tea. Using this idea, they obtained promising results in image generation, but since GANs produced more convincing images back then, diffusion models did not get as much attention.

Then, in 2020, [Jonathan Ho et al.](https://arxiv.org/abs/2006.11239), also from UC Berkeley, managed to build a diffusion model capable of generating highly realistic images, which they called a *denoising diffusion probabilistic model* (DDPM). A few months later, a [2021 paper](https://arxiv.org/abs/2102.09672) by OpenAI researchers Alex Nichol and Prafulla Dhariwal analyzed the DDPM architecture and proposed several improvements that allowed DDPMs to finally beat GANs: not only DDPMs are much easier to train than GANs, but the generated images are more diverse and of even higher quality. The main downside of DDPMs, as you'll see, is that they take a very long time to generate images, compared to GANs and VAEs.

How exactly does a DDPM work? Suppose you start with an image of cat (like the one below), noted $\textbf{x}_0$, and at each time step $t$, you add a bit of Gaussian noise to the image ,with mean 0 and variance $\beta_t$. This noise is independent for each pixel: we call it *isotropic*. You first obtain the image $\textbf{x}_1$, then $\textbf{x}_2$, and so on until the cat is completely hidden by the noise, impossible to see. The last time step is noted $T$. In the original DDPM paper, the authors used $T = 1,000$, and they scheduled the variance $\beta_t$ in such a way that the cat signal fades linearly between time step 0 and $T$. In the improved DDPM paper, $T$ was increased to 4,000, and the variance was tweaked to change more slowly at the end and the beginning. In short, we're gradually drowning the cat in noise: this is called the *forward process*.

As we add more and more Gaussian noise in the forward process, the distribution of pixel values becomes more and more Gaussian. The pixel values also get scaled slightly at each step, by a factor of $\sqrt{1 - \beta_t}$. This ensures that the mean of the pixel values gradually approach 0, since the scaling factor is a bit smaller than 1 (imagine repeatedly multiplying by 0.99). It also ensures that the variance will gradually converge to 1. THis is because the standard deviation of the pixel values also get scaled by $\sqrt{1 - \beta_t}$, so the variance get scaled by $1 - \beta_t$ (i.e., the square of the scaling factor). But the variance cannot shrink to 0 since we're adding Gaussian noise with variance $\beta_t$ at each step. And since the variances add up when you sum two probability distribution, you can see that the variance can only converge to $1 - \beta_t + \beta_t = 1$.

The forward diffusion process is summarized in the equation below:
    $$q(\textbf{x}_t| \textbf{x}_{t-1}) = \mathcal{N}(\sqrt{1-\beta_t} \textbf{x}_{t-1}, \beta_t\textbf{I})$$

This equation is just the mathematical formalization of what we talked about previously. This equation defines the probability distribution $q$ of $\textbf{x}_t$ given $\textbf{x}_{t-1}$ as a Gaussian distribution with mean $\textbf{x}_{t-1}$ times the scaling factor, and with a covariance matrix of $\beta_t\textbf{I}$. This is the identity matrix $\textbf{I}$ multiplied by $\beta_t$, which means the noise is isotropic with variance $\beta_t$.

Interestingly, there's a shortcut for the forward process: it's possible to sample an image $\textbf{x}_t$ given $\textbf{x}_0$ without having to compute $\textbf{x}_1$, $\textbf{x}_2$, ..., $\textbf{x}_{t-1}$. Indeed, since the sum of multiple Gaussian distribution is also a Gaussian distribution, all the noise can be added in one shot. This is the equation we will be suing, which is much faster:
    $$q(\textbf{x}_t| \textbf{x}_0) = \mathcal{N}(\sqrt{\bar{\alpha}_t} \textbf{x}_0, (1-\bar{\alpha}_t)\textbf{I})$$

Our goal is not do drown cats in noises! It's quite the opposite, we want to create more cats. We can do so by training a model that can perform the *reserve process*: going from $\textbf{x}_t$ to $\textbf{x}_{t-1}$. We can then use it to remove a tiny bit of noise from an image, and repeatedly do so until all the noise is gone. If we train the model on a dataset of cat images, then we can feed it a picture entirely of Gaussian noise, and the model will gradually make a brand new cat appear.
![The forward process q and reverse process p](image-9.png)

The first thing we need to do is code the forward process. For this, we will need to implement the variance schedule. How can we control how fast the cat disappears? Initially, 100% of the variance comes from the original cat image. Then at each time step $t$, the variance gets multiplied by $1 - \beta_t$, as explained earlier, and noise gets added. So the part of the variance that comes from the initial distribution shrinks by a factor of $1 - \beta-t$ at each step. If we define $\alpha_t = 1 - \beta_t$, then after $t$ time steps, the cat signal will be multiped by a factor of $\bar{\alpha_t} = \alpha_1 \times \alpha_2 \times \dots \times \alpha_t$ or $\bar{\alpha_t} = \prod_{i=1}^t \alpha_i$. It is this "cat signal factor" $\bar{\alpha_t}$ that we want to schedule so it shrinks down from 0 to 1 gradually between time step 0 and T. In the improved DDPM paper, the authors schedule $\bar{\alpha_t} $ as followed.
    $$\beta_t = 1 - \dfrac{\bar{\alpha}_t}{\bar{\alpha}_t-1}, \text{ with } \bar{\alpha}_t = \dfrac{f(t)}{f(0)} \text{ and } f(t) = \cos\left(\dfrac{t/T+s}{1+s}.\dfrac{\pi}{2} \right)^2$$
In these equations:
- s is a tiny value that prevent $\beta_t$ from being too small near $t = 0$. In the paper, the authors used $s = 0.008$.
- $\beta_t$ is clipped to be no larger than 0.999, to avoid instabilities near $t = T$.

To train our model to reserve the diffusion process, we will need noisy images from different time steps of the forward process. For this, we will create a `prepare_batch()` function that will take a batch of clean images from the dataset and prepare them. We'll go through the implementation of it:
- For simplicity, we will use Fashion MNIST, so the function must add add a channel axis. It will also help to rescale the pixel values from -1 to 1, so it's close to the final Gauss distribution with mean 0 and variance 1.
- Next, the function creates `t`, a vector contains a random time step for each image in the batch, between 1 and $T$.
...
- The next line reshapes the `alpha_cm` from [*batch size*] to [*batch size*, 1, 1, 1]. This is needed to ensure `alpha_cm` is broadcasted properly with all instance in the batch `X`.
- Then we generated some Gaussian noise with mean 0 and variance 1.
- Lastly, we use the shortcut equation to apply the diffusion process to the images. Note that `x ** 0.5` is equal to the square root of `x`. The function returns a tuple containing the inputs and the targets. The inputs are represented as a Python's dictionary containing the noisy images and the time steps used to generate them. The targets are the Gaussian noise used to generate them. The targets are the Gaussian noise used to generate each image.

With this setup, the model will predict the noise that should be subtracted from the input image to get original. Why not predict the original image directly? Well, the authors tried: it simply doesn't work as well.

Next, we'll create a training dataset and a validation set that will apply this `prepare_batch()` function to every batch. As earlier, `X_train` and `X_valid` contain the Fashion MNIST images with pixel values ranging from 0 to 1.
...
The DDPM authors used a modified [U-Net architecture](https://arxiv.org/abs/1505.04597), which has many similarities with the FCN architecture we discussed in chapter 14 for sematic segmentation: it's a a convolutional neural network that gradually downsamples the input images, then gradually upsamples them again, with skip connections crossing over from each level of the downsampling part to the corresponding level in the upsampling part. To take into account the time steps, they encoded them using the same technique as the positional encodings in the transformer architecture (in chapter 16). At every level in the U-Net architecture, they passed these time codings through a `Dense` layer and fed them back to the U-net. See the learning notebook for an example, or [this link](https://github.com/hojonathanho/diffusion/blob/master/diffusion_tf/models/unet.py) for the official implementation: it's based on TF 1.X, which is deprecated, but it's quite readable.

We can train this model normally. The author noted that the MAE loss is better than the MSE loss. You can also use the Huber loss.

Once the model is trained, you can use it to generate new images. Unfortunately, there is no shortcut for this reverse diffusion process: you have to sample $\textbf{x}_T$ randomly from a Gaussian distribution with mean 0 and variance 1, then pass it to the model to predict the noise; subtract it from the image using the equation below to get $x_{T-1} $:
    $$x_{t-1} = \dfrac{1}{\sqrt{\alpha_t}}\left(x_t - \dfrac{\beta_t}{\sqrt{1 - \bar{\alpha}_t}}\epsilon_\theta(\textbf{x}_t, t)\right) + \sqrt{\beta_t}\textbf{z}$$
Repeat this process 3,999 more times until you get $\textbf{x}_0$: if all went well, it should look like the regular MNIST image.

In this equation, $\epsilon_\theta (\textbf{x}_t, t)$ is the noise predicted by model given the input image $\textbf{x}_t$ and the time step $t$. The $\theta$ represents the model parameters. Moreover, $\textbf{x}_t$ is the Gaussian noise with mean 0 and variance 1. This makes the reserve process stochastic: if you run it multiple times, you will get different images.

The generation of new images may take a minute or two. That's the main drawback of the diffusion models generating images is slow since the model needs to be called many times. It's possible to make this faster by using a smaller T value, or by using the same model prediction for several steps at a time, but the resulting image may not look as nice. That said, despite this speed limitation, diffusion models do produce high-quality and diverse images.

Diffusion models have made tremendous progress recently. In particular, a paper published in December 2021 by Robin Rombach, Andreas Blattmann, et al., introduced *latent diffusion models*, where the diffusion process takes place in latent space, instead of pixel space. This requires a powerful autoencoder to compress each training image into a much smaller latent space, where the diffusion process takes place, then the autoencoder is used to decompress the final latent, generating the output image. This considerably speeds up image generation, and reduces the training time and cost significantly. Importantly, the quality of the generated images are outstanding.

Moreover, the researchers also adapted various conditioning techniques to guide the diffusion process, such text prompts, images, or any other inputs. This makes it possible to quickly produce a beautiful, high-resolution image of anything you want. You can also condition the image generation process using an input image. This enables many applications, such as outpainting - where an input image is extended beyond its borders - or inpainting - where holes in an image are filled in.

Lastly, a powerful pretrained latent diffusion model named *Stable Diffusion* was open sourced in August 2022 by a collaboration between LMU Munich and a few companies, including StabilityAI, and Runway, with support from EleutherAI and LAION. In September 2022, it was ported to TensorFlow and included in Keras CV, a computer vision library built by the Keras team. You can easily generate mind-blowing images in seconds for free, even in a regular laptop (see the last exercise of this chapter).