...
**Note**: The rest of this notebook is appendix D, as it has more detail than in chapter 12.
# Appendix D

- In this Appendix, we will explore the graphs generated by TF functions.

## TF Functions and Concrete Functions

- TF functions are polymorphic, which means they support inputs of different types (and shapes).
- For example, consider the `tf_cube()` function we created before.
...
- Such a combination of argument types and input shapes is called an `input signature`.
- If you call a TF function with an input signature it has already seen before, it will reuse the same concrete function it generated earlier.
- For example, if you call `tf_cube(tf.constant(3.0))`, and it will reuse the same concrete function it used for `tf_cube(tf.constant(2.0))` (for float32 scalar tensors).
- But it will generate a new concrete function if you call `tf_cube(tf.constant([3.0]))` or `tf_cube(tf.constant([2.0]))` (float32 tensors of shape [1]), and yet another for `tf_cube(tf.constant([[1.0, 2.0], [3.0, 4.0]]))` (for float32 of tensors of shape [2, 2]).
...
- We can find two images in the book, each of them shows the `tf_cube()` TF function, after we called `tf_cube(2)` and `tf_cube(tf.constant(2))`: two concrete functions were generated, one for each signature, each with its own optimized *function graph* (`FuncGraph`) and its own *function definition* (`FunctionDef`):
    - A function definition points to the parts of the graph that correspond to the function's inputs and outputs.
    - In each `FuncGraph`, the nodes represents operations (e.g., power, constant, or placeholders for arguments like `x`), while the edges (the solid arrows between the operations) represent the tensors that will flow through the graph.
...
- The concrete function on the right is specialized for float32 scalar tensors, and it could not be simplified anymore.
...
- Most of the times, converting a Python functions that performs TensorFlow operations into a TF function is trivial: decorate it with `@tf.function` or let Keras takes care of it for you.
- However, there're some rules:
    - If you call any external libraries, including NumPy and even the standard library, this call will only run during tracing; it will not be part of the graph. In fact, a TensorFlow graph can only include TensorFlow constructs (tensors, operations, variables, datasets, etc.). So make sure you use `tf.reduce_sum()` instead of `np.sum()` and `tf.sort()` instead of the built-in `sorted` function, and so on (unless you want to the code to only run during tracing). This has a few applications:
        - If you define a TF function `f(x)` that just returns `np.random.rand()`, a random number will only be returned when the function is traced, so `f(tf.constant(2))` and `f(tf.constant(3))` will return the same random number, but `f(tf.constant([2, 3]))` will return a different one. If you replace `np.random.rand()` with `tf.random.uniform()`, then a new random number will be generated upon every call, since the operation will be part of the graph.
        - If you non-TensorFlow code has side effects (such as logging something or updating a Python counter), then you should not expect those side effects every time you call TF function, as they will only occur when the function is traced.
        - Yu cna wrap any Python function in a `tf.py_function()` operation, but doing so hurt the performance, as TensorFlow will not be able to any graph optimization in this code. It will also reduce portability, as the graph will only run on platforms where Python is available (and the needed libraries are installed).
    ...
    - If the function creates a TensorFlow variable (or any other stateful TensorFlow object, such as a dataset or a queue), it must do so upon the first call and only then, or else you will get an exception. It is usually preferable to create variables outside of the TF function (e.g., in the `build()` method of a custom layer). If you want to assign a new value to a variable, make sure you use `assign()` instead of using the `=` operator.
    - The source code of your Python function should be available to TensorFlow. If your source code is unavailable (e.g., you define your function in the Python shell, which does not give access to the source code, of if you deploy only the complied _*.pyc_ to production), then the graph generation process will fail or have limited functionalities.
    - TensorFlow will only capture `for` loops that iterate over a tensor or a `tf.data.Dataset`. Therefore, make sure you use `for i in tf.range(x)` rather than `for i in range(x)`, or else the loop will not be captured in the graph. Instead, it will only run during tracing. This may be what you want you want if the `for` loop is meant to build the graph; for example, when creating each layer in a neural network.
    - As always, for performance reason, always favor a vectorized implementation than using loops, whenever you can.